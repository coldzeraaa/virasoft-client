...
export function ProductSingle({
  ...
  return (
    <Link
      href={`/product/${slug}`}
      className="group flex h-40 w-full flex-col md:h-96"
    >
      ...
      <div className="mt-2 h-1/6">
        ...
        <div className="flex items-center space-x-2">
          <p
            className={`font-medium md:text-lg ${is_sale ? "text-accent" : "text-primary"}`}
          >
        ...
        {is_sale && (
          <div className="text-sm font-medium text-primary">
            {Math.round(100 - (100 * (selling_price || price)) / price)}% off
          </div>
        )}
      </div>
    </Link>
