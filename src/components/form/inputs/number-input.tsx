      ...
      {Array.from({ length }).map((_, idx) => (
        <input
          ...
          onKeyDownCapture={(event) => {
            if (event.key === "Backspace" && !values[idx] && idx - 1 >= 0)
              refs[idx - 1]?.current?.focus();
            if (event.key === "ArrowLeft" && idx - 1 >= 0)
              refs[idx - 1]?.current?.focus();
            if (
              event.key !== "Backspace" &&
              event.key !== "ArrowLeft" &&
          ...
          onChange={(event) => {
            if (event.target.value.length > 1)
              return onChange(event.target.value.slice(0, length));
            if (idx === length - 1 && event.target.value)
              (event.target as AnyType).select();
          ...
          onPaste={(event) => {
            ...
            refs[
              paste.length > length ? length - 1 : paste.length
            ]?.current?.focus();
          }}
