    ...
    <>
      <ul className="space-y-2">
        {data.me.orders.edges.map(({ node }) => (
          <li
            key={node.id}
            className="rounded-lg bg-base-200 p-4 transition-all hover:shadow"
          >
            <SingleOrder {...node} />
          </li>
        ))}
      ...
      {data.me.orders.pageInfo.hasNextPage && (
        <div
          ...
          onClick={() =>
            fetchMore({
              variables: { after: data?.me?.orders.pageInfo.endCursor },
            })
          }
        >
          <button
            type="button"
            className="btn btn-primary btn-sm btn-wide mx-auto block"
          >
            Fetch more
          </button>
        </div>
