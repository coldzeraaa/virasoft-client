...
export function SingleOrder({ number, createdAt, itemCount, total, items }: SingleOrderProps) {
  return (
    <div className="grid grid-cols-12">
      ...
      <ul className="avatar-group col-span-4 flex-1 -space-x-6 overflow-visible p-1">
        {items.map((item, idx) => (
          <li key={item.id} className="group tooltip tooltip-left bottom-0 cursor-pointer" data-tip={item.variant.product.name}>
            <Link href={`/product/${item.variant.product.slug}`}>
              <Image
                ...
                className={`relative bottom-0 aspect-square w-10 rounded-full object-cover ring  ${
                  items.length > 1 && items.length !== idx + 1 ? 'transition-all group-hover:bottom-8' : ''
                }`}
              />
            </Link>
          </li>
