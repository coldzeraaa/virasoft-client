    ...
    <div className="w-full rounded-2xl  shadow-lg hover:shadow-xl">
      <div className=" flex w-full justify-between px-4 py-6">
        <div>
          ...
          <div className="text-lg">{data?.myOrder?.number}</div>
        </div>
        <div>
          <div className="text-gray-500"> Захиалга хийсэн огноо</div>
          <div className="text-lg">{new Date(data?.myOrder?.createdAt).toLocaleDateString()}</div>
        </div>
      </div>
      <hr />
      <div className="flex flex-col   px-4 py-2">
        <div className="flex justify-between">
          ...
          <div className="text-lg">{data?.myOrder?.itemTotal}₮</div>
        </div>
      ...
      <div className="flex justify-between border-t-2 border-dotted px-4 py-2">
        ...
        <div className="text-lg">{data?.myOrder?.total}₮</div>
      </div>
    </div>
  );
};
