...
} from "@/gql/query/order/current-order.generated";
...
export const useCurrentOrder = () =>
  useContext<CurrentOrderContextProps>(CurrentOrderContext);

export const CurrentOrderProvider: FC<PropsWithChildren> = ({ children }) => {
  const [number, token] =
    typeof window !== "undefined"
      ? localStorage.getItem("order")?.split(":") || []
      : [];
  const variables = number ? { number, token } : undefined;
  const { data, loading } = useCurrentOrderQuery({
    variables,
  ...
  return (
    <CurrentOrderContext.Provider
      value={{ order: data?.currentOrder, loading }}
    >
      {children}
    </CurrentOrderContext.Provider>
  );
