...
export function Footer() {
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[0.6fr_0.4fr_0.3fr] w-full">
          <div className="flex h-full w-full lg:justify-end bg-primary py-20 px-20 xl:pl-48">
            <div className="space-y-4 rounded-l-xl bg-primary">
              ...
              {contactMenu?.children?.map((menuItem, idx) => (
                <div key={idx} className="flex items-center gap-4">
                  ...
                  <Link href={menuItem.link} className="text-base-100">{menuItem.title}</Link>
                </div>
              ))}
          ...
          <div className="xl:px-20 px-20 lg:px-10 pt-20">
            <h2 className="text-2xl font-semibold text-accent mb-4">Хамтарч ажиллах</h2>
            <ContactForm/>
          </div>
          <div className='p-20 lg:px-10 xl:p-20 flex flex-col gap-14'>{data?.allMenus?.nodes[1]?.children?.filter((menu) => menu.title !== 'Бидэнтэй холбогдох').map((menuItem) => (<Link href={menuItem.link} className='text-sm font-semibold text-neutral/70'>{menuItem.title}</Link>))}</div>
        </div>
      </footer>
    </>
  );
