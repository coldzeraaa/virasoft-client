...
export function PaymentSection({
  ...
  const [updateCheckoutAddress, { loading: checkoutAddressLoading }] =
  ...
  const [paymentCheckout, { loading: paymentCheckoutLoading, error }] =
  ...
  if (loading || paymentCheckoutLoading || checkoutAddressLoading)
    return <div className="skeleton h-20 w-full" />;
  if (!order)
    return <div className="h-60 w-full">Танд захиалга байхгүй байна</div>;
    ...
    <>
        ...
        {order?.items.map((item) => (
          <li key={item.id}>
            <p className="grid grid-cols-[1fr,auto,auto] items-center gap-2">
              <span className="t-text-xs line-clamp-2 truncate">
                {item.variant.product.name}
              </span>
              <span className="t-text-xs h-fit rounded bg-base-300 p-1 px-2">
                {item.quantity}ш
              </span>
              <span className="t-text-xs min-w-20 text-right">
      ...
      <p className="mb-6 flex items-center justify-between border-t border-dashed py-3 font-bold">
        ...
        <span className="heading-4">
          {moneyFormatHelper(order?.total || 0)}
        </span>
      </p>
      <button
        ...
        onClick={() => {
          if (pathName === "/checkout/address") {
            if (!selectedAddress)
              return toast.error("Хүргэлтийн хаягаа оруулна уу");
            return updateCheckoutAddress({
              variables: {
                input: { shipAddressId: selectedAddress, number: "-" },
              },
            });
          }
          if (pathName === "/checkout/review")
            return paymentCheckout({
              variables: {
                input: { action: PaymentMethodEnum.VirasoftPay, number: "-" },
              },
            });
