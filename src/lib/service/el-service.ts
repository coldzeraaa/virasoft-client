...
export async function elService(props: ElServiceProps): Promise<HitType[]> {
  return fetch(`${HOST_CONFIG.elastic.host}/${APP_CONFIG.appIndex}/_search`, {
    ...
    .then(
      (response) =>
        response &&
        response.hits.hits.map((hit: { _source: HitType; _id: string }) => ({
          ...hit._source,
          id: hit._id,
        })),
    );
...
function getMusts(props: GetMustProps): QueryStringType[] {
  return Object.entries(props).reduce((acc: QueryStringType[], cur) => {
    ...
    if (key === "skuString")
      return getSkuTerms(
        acc,
        value as NonNullable<ElServiceProps["skuString"]>,
      );
    if (key === "ids")
      return getIds(acc, value as NonNullable<ElServiceProps["ids"]>);
    if (key === "slugs")
      return getSlugs(acc, value as NonNullable<ElServiceProps["slugs"]>);

    return getQueryString(acc, value as string | string[], key);
  }, []);
}

function getSkuTerms(
  acc: QueryStringType[],
  value: ElServiceProps["skuString"],
): QueryStringType[] {
  return [...acc, { terms: { SKU: getTrimmedArrayHelper(value) } }];
}
function getSlugs(
  acc: QueryStringType[],
  value: ElServiceProps["slugs"],
): QueryStringType[] {
  if (!value) return [];
...
function getQueryString(
  ...
  return [
    ...acc,
    { [Array.isArray(value) ? "terms" : "term"]: { [key]: value } },
  ];
}
