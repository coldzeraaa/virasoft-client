  ...
  return (
    <>
      <nav className="flex-1 overflow-auto">
        <Link href="/account" className={`btn btn-ghost btn-block justify-start ${pathname === '/account' ? 'btn-active' : ''}`}>
          <House />
          Миний самбар
        </Link>
        <Link
...
function Logout() {
  ...
  return (
    <button onClick={logout} type="button" className="btn btn-block justify-start">
      <LogOutIcon />
      Гарах
    </button>
  );
