...
export function LastOrders() {
  ...
  return (
    <div className="mt-8 h-full w-full flex flex-col justify-between gap-4 lg:flex-row">
      {data?.me?.orders?.nodes.slice(0, 2).map((order, idx) => (
        <Link
          href={`/account/orders/${data?.me?.orders.nodes[idx].number}`}
          key={idx}
          className="flex w-full h-fit gap-6 rounded-lg border bg-base-100 p-4 transition-all duration-100 hover:cursor-pointer hover:bg-base-200 lg:p-6"
        >
